...
symbols=['LPTUSDT','WIFUSDT','STRKUSDT','LUNAUSDT','SOLUSDT','AVAXUSDT','ORDIUSDT','FILUSDT','SANTOSUSDT','CYBERUSDT','DEXEUSDT','PROMUSDT','ARUSDT','ERNUSDT','LAZIOUSDT','CITYUSDT','RNDRUSDT']
...
log_path = './log/001-3.txt'
sleeptime = 66/len(symbols)*15
while True:
    for symbol in symbols:
        try:
            ...
            if (sum_of_negative_numbers(magnitude) < fall_goal and magnitude0 > fall_goal * 0.5 and magnitude0 < 0.008) or magnitude0+magnitude1 < fall_goal:
                ...
                ifbuy = if_near_highprice_to_buy(client.get_ticker(symbol=symbol), buy_price,dis=1/5)
                if not ifbuy:
                    continue
